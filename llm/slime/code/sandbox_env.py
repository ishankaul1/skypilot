...
class SandboxEnvironment:
    ...
    def __init__(self, sb, *, config_class: type = SandboxEnvironmentConfig, **kwargs) -> None:
        self.sb = sb
    ...
    def execute(self, action: dict, cwd: str = "", *, timeout: int | None = None) -> dict[str, Any]:
        ...
        try:
            handle = self.sb.exec(
                ...
                command,
                workdir=cwd,
                timeout_seconds=float(timeout),  # server-side kill: no orphaned commands
            ...
            output_text = (
                (out_b.decode("utf-8", "replace") if isinstance(out_b, bytes) else out_b)
                + (err_b.decode("utf-8", "replace") if isinstance(err_b, bytes) else err_b)
            )
            output = {"output": output_text, "returncode": returncode, "exception_info": ""}
    ...
    def write_files(self, files: dict[str, str] | None) -> None:
        ...
        for path, content in (files or {}).items():
            dst = path if path.startswith("/") else f"{self.config.cwd.rstrip('/')}/{path}"
...
async def claim_sandbox(
    ...
    *,
    ...
    """
    if _sandbox is None:
        raise RuntimeError("sky.sandbox SDK not installed (skypilot_sandbox_sdk wheel)")
    ...
    if _NO_POOL:
        ...
        sb = await _sandbox.create.aio(name=name, image=image, cpus=1, memory_gb=2,
                                       timeout=lifetime_sec)
    else:
        try:
            sb = await _sandbox.create.aio(name=name, pool=pool, timeout=lifetime_sec)
            ...
            if image is None:
                raise
            print(f"[sandbox] pool {pool!r} claim failed ({e}); falling back to "
                  f"on-demand sandbox {name!r}", flush=True)
            sb = await _sandbox.create.aio(name=name, image=image, cpus=1, memory_gb=2,
                                           timeout=lifetime_sec)
    if workdir:
        try:
...
def ensure_pool(name: str, *, image: str, replicas: int, cpus: float = 1, memory_gb: float = 2) -> None:
    ...
    try:
        _sandbox.create_pool(name=name, image=image, cpus=cpus, memory_gb=memory_gb, replicas=replicas)
        dt = time.perf_counter() - t0
        print(f"[pool-timing] created {name!r} image={image} replicas={replicas} create_call={dt:.2f}s", flush=True)
        return
    except Exception as e:  # noqa: BLE001
        dt = time.perf_counter() - t0
        msg = str(e)
        ...
        print(f"[pool-timing] {name!r} not confirmed at full size ({msg}) in {dt:.2f}s; "
              f"ensuring size {replicas} and proceeding (overflow claims fall back on-demand)", flush=True)
        try:
            _sandbox.set_pool_size(name, replicas=replicas)
        except Exception as e2:  # noqa: BLE001
            print(f"[pool-timing] set_pool_size({name!r}) did not confirm full warm ({e2}); "
                  f"proceeding on the partial pool", flush=True)
