...
def _collect_cluster_kubernetes_resources(
    ...
    # Resolve the cluster's k8s coordinates from its command runners. For a
    # Kubernetes cluster every runner is a KubernetesCommandRunner carrying its
    # pod's (context, namespace, name); runners[0] is the head. This reuses the
    # request-scoped cache populated by the skylet-log collection, so it's a
    # cheap call here.
    try:
        runners = handle.get_command_runners()
    except Exception as e:  # pylint: disable=broad-except
    ...
    pod_names = [r.pod_name for r in k8s_runners]
